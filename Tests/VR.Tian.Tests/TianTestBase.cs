...
namespace VR.Tian.Tests
{
    public abstract class TianTestBase : AbpIntegratedTestBase<TianTestModule>
    {
        ...
        protected void UsingDbContext(Action<TianDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<TianDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<TianDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<TianDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<TianDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TianDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<TianDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TianDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<TianDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<TianDbContext, Task<T>> func)
        {
            T result;
